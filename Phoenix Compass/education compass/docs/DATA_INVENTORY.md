...
核对日期：2026-09-23。对应 schema：`server/migrations/001` 至 `007`。
...
| 4 | 注销后备份中的 30 天残留如何告知？ | 注销功能已上线（4.2），但备份不回溯清理（4.4） |
...
> 用户可自助注销账号并删除全部个人数据，订单等财务凭证按法规保留但已与个人数据脱钩；
> **备份中的残留最多 30 天，且目前没有数据导出能力**。
