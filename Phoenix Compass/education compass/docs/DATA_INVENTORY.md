...
核对日期：2026-09-22。对应 schema：`server/migrations/001` 至 `006`。
...
> 核对：服务端全部 DELETE 路由共 5 条，见 `server/src/http/app.ts` 与 `server/src/http/agent-routes.ts`
> （`grep -cE "'DELETE'" server/src/http/app.ts server/src/http/agent-routes.ts`）。

### 4.2 ⚠ 当前**没有**的能力

- **没有账号注销接口**。没有任何接口可以删除 `users`、`families`、`students`、`assessments` 或 `reports`。
  外键是 `ON DELETE CASCADE`，数据库层面支持级联删除，但**没有任何产品路径触发它**。
- **没有个人信息导出接口**。
- 撤回同意会阻止后续使用，但**不删除已有记录**。

这三项直接关系到法定的删除权与可携带权。隐私政策在写"如何删除您的信息"之前，这些能力必须先存在。

### 4.3 备份中的残留

数据库转储保留 14 天（本地）与 30 天（COS）。即使将来实现了账号注销，
